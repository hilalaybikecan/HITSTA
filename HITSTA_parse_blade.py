...
def import_HITSTA(filenames):
    ...
    exp = {}
    for section in sections_merged[1:]:
        ...
        exp[IDstring] = {
            ...
            / np.tile(df[df["Type (D/W/L)"] == "D"].iloc[:, 3].to_numpy(dtype=float), (len(wavelengths), 1)).transpose(),
            "Laser Raw": df[df["Type (D/W/L)"] == "L"].iloc[:, 4:].to_numpy(dtype=float)
            / np.tile(df[df["Type (D/W/L)"] == "L"].iloc[:, 3].to_numpy(dtype=float), (len(wavelengths), 1)).transpose(),
        }
        if 'W' in df["Type (D/W/L)"].to_list():
            exp[IDstring]["Reflectance Raw"] = df[df["Type (D/W/L)"] == "W"].iloc[:, 4:].to_numpy(dtype=float)/ np.tile(df[df["Type (D/W/L)"] == "W"].iloc[:, 3].to_numpy(dtype=float), (len(wavelengths), 1)).transpose()
    ...
    for IDstring in exp.keys():
        ...
        has_reflectance = "Reflectance Raw" in exp[IDstring].keys()
    ...
    for IDstring in exp.keys():
        ...
        ind_max = len(exp[IDstring]["Times"])-1
...
if __name__ == "__main__":
    ...
    exp = import_HITSTA(r"G:\My Drive\Local Experiments\T548 - HC - Int mix_10x10_bayesian1\HITSTA\Mix intermediates_aging(2).txt")
...
#%% 
    for ID in exp.keys():
        fig, ax = plt.subplots()
        for time_index in range(len(exp[ID]["Times"])):
            ax.plot(exp[ID]["Wavelengths"], exp[ID]["PL Subtr"][time_index, :],label =f'Time {exp[ID]["Times"][time_index]:.2f} h')
            ax.plot(exp[ID]["Wavelengths"], exp[ID]["PL Fitted"][time_index, :],'k--')
        ax.legend()
        plt.xlim([550,900])
        ax.set_title(ID)
...
colormap = np.linspace(0,1,80)
colors = plt.cm.viridis(colormap)  

excluded_ids = ["ID1", "ID2"]
# included_ids =["ID22","ID23","ID24","ID25"]

# for idx, ID in enumerate(included_ids):
for idx, ID in enumerate(exp.keys()):
    if ID in excluded_ids:
        continue
    
    # for rnd in exp[ID]["Rounds"]:
    for rnd in [0]:
        plt.plot(exp[ID]["Wavelengths"],exp[ID]["PL Subtr"][rnd], color =colors[idx],label=str(ID), linewidth=1.5)
        # plt.plot(exp[ID]["Wavelengths"],exp[ID]["PL Subtr"][rnd], color =colors[idx],label=labels[idx], linewidth=1.5)
    # plt.plot(exp[ID]["Wavelengths"],exp[ID]["PL Fitted"][rnd],'k--')


plt.xlim([650,900])
plt.legend(loc="upper right", fontsize =6)
plt.ylabel ("PL intensity (initial)", fontsize = 10)
plt.xlabel ("Wavelength (nm)", fontsize = 10)
# plt.title("Aging : 85C 2-sun")
...
# fig, ax = plt.subplots(figsize=(6*cm, 4*cm), dpi=600)
fig, ax = plt.subplots(dpi=600)
colormap = np.linspace(0,1,80)
...
for idx, ID in enumerate(exp.keys()):   
    if ID in excluded_ids:
        continue
    # inds = (exp[ID]["Times"]>26) | (exp[ID]["Times"]<16)
    if exp[ID]["PL Fit Parameters"][0,0]>0.0:
        plt.plot(exp[ID]["Times"], exp[ID]["PL Fit Parameters"][:,0], 'o-', linewidth=2,
                 label=ID, markersize=4, color=colors[idx])
        # if not all(np.isnan(exp[ID]["PL Lifetime Fit"]).flatten()):
        # plt.plot(exp[ID]["PL Lifetime Fit"][0], exp[ID]["PL Lifetime Fit"][1], '--', linewidth=1.5, markersize=3, color="#650021",label='fit')
        # plt.plot(exp[ID]["Times"][inds], plfit_func(exp[ID]["Times"][inds],8,0.3,0.5,5), '--', linewidth=1.5, markersize=4, color=colors[idx])
        # plt.plot(exp[ID]["Times"][inds], 5*exp[ID]["PL Self-Similarity"][inds], '--', linewidth=1.5, markersize=4, color=colors[idx],label=ID)
    
# plt.ylim(0,1.1*max(exp[ID]["PL Fit Parameters"][:,0]))
...
# plt.xticks((np.arange(0,201,50)),fontsize=5)
# plt.yticks((np.arange(0,11,2)),fontsize=5)
# plt.yscale("log")
# plt.savefig("Fig5e.svg", format="svg")
